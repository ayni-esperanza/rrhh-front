          ...
          <thead class="bg-slate-50 text-[11px] font-bold text-slate-700 dark:bg-slate-800 dark:text-slate-300">
            <tr>
              ...
              @for (dia of dias; track dia.dia) {
                <th class="px-3 py-3 text-center">{{ dia.dia }} {{ dia.fecha }}</th>
              }
              ...
              <tr class="hover:bg-slate-50/70 dark:hover:bg-slate-800/50">
                ...
                @for (dia of item.dias; track dia.dia) {
                  <td class="px-3 py-3 text-center">
                    <span class="inline-flex min-w-20 justify-center rounded-md px-2 py-1 font-semibold" [class]="badgeClasses(dia.tipo)">{{ dia.valor }}</span>
                  </td>
                }
...
export class LugarTrabajoPageComponent {
  ...
  protected readonly registros: LugarSemana[] = this.colaboradores.map((item, index) => ({
    id: item.id,
  ...
  private place(dia: string, fecha: string, valor: string, tipo: LugarTipo): LugarDia {
    return { dia, fecha, valor, tipo };
  }

  private empty(dia: string, fecha: string): LugarDia {
    return { dia, fecha, valor: '-', tipo: 'vacio' };
  }
}
