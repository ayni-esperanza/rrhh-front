  ...
  template: `
    <section class="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
      <header class="flex flex-col gap-3 border-b border-slate-200 px-4 py-4 dark:border-slate-800 lg:flex-row lg:items-center lg:justify-between">
        <div>
          <h2 class="text-sm font-bold text-slate-950 dark:text-white">Hora de entrada y salida</h2>
        </div>
            ...
            @for (item of registros; track item.id) {
              <tr class="hover:bg-slate-50/70 dark:hover:bg-slate-800/50">
                ...
                @for (dia of item.dias; track dia.dia) {
                  @if (dia.turno === 'falta') {
                    <td class="px-2 py-3 text-center font-bold text-red-600" colspan="2">- Falta</td>
                  } @else {
                    <td class="px-2 py-3 text-center"><span class="inline-flex items-center gap-1.5" [class]="textClasses(dia.turno)"><span class="h-2 w-2 rounded-full" [class]="dotClasses(dia.turno)"></span>{{ dia.entrada }}</span></td>
                    <td class="px-2 py-3 text-center"><span [class]="textClasses(dia.turno)">{{ dia.salida }}</span></td>
                  }
                }
...
export class EntradaSalidaPageComponent {
  ...
  protected textClasses(turno: Turno): string {
    const classes = {
      atiempo: 'text-slate-800 dark:text-slate-200',
      tarde: 'font-semibold text-orange-600 dark:text-orange-300',
      falta: 'font-semibold text-red-600 dark:text-red-300',
  ...
  private day(dia: string, fecha: string, entrada: string, salida: string, turno: Turno = 'atiempo'): EntradaSalidaDia {
    return { dia, fecha, entrada, salida, turno };
  }

  private empty(dia: string, fecha: string): EntradaSalidaDia {
    return { dia, fecha, entrada: '-', salida: '-', turno: 'vacio' };
  }

  private missing(dia: string, fecha: string): EntradaSalidaDia {
    return { dia, fecha, entrada: '-', salida: '-', turno: 'falta' };
  }
}
