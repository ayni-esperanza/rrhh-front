...
export interface ReglaPagoFeriado { origen: 'ESPECIFICA' | 'PREDETERMINADA'; configuracion: { tipoCalculo: string; valor: number }; }
export interface CalculoPagoFeriado { montoCalculado: number; origen: 'ESPECIFICA' | 'PREDETERMINADA'; }
...
export class FeriadosService {
  ...
  getRule(fecha: string) { return this.http.get<ReglaPagoFeriado>(`${this.url}/fecha/${fecha}/regla`); }
  ...
  calculatePayment(id: string, remuneracionMensual: number, horasTrabajadas: number) { return this.http.post<CalculoPagoFeriado>(`${this.url}/${id}/calcular-pago`, { remuneracionMensual, horasTrabajadas }); }
}
