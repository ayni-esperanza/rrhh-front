...
import { AlertasMetrics, AlertasService } from '../../services/alertas.service';
...
export class AlertasPageComponent {
  ...
  protected metrics: AlertasMetrics | null = null;
  ...
  protected generateAlerts(): void {
    this.generating = true;
    this.alertasService.generate().subscribe({ next: () => { this.generating = false; this.reload(); }, error: () => this.generating = false });
  }
  ...
  protected markAsSeen(alerta: Alerta): void { this.alertasService.markAsSeen(alerta.id).subscribe(() => alerta.visto = true); }
  protected markFilteredAsSeen(): void { this.alertasService.markAllAsSeen().subscribe(() => this.alertas.forEach((alerta) => alerta.visto = true)); }

  protected typeLabel(type: Alerta['tipo']): string {
    return { inasistencia: 'Inasistencia', pago: 'Pago', cumpleanos: 'Cumpleaños', antiguedad: 'Tiempo de trabajo', documento: 'Documento' }[type];
  }
  ...
  protected typeClasses(type: Alerta['tipo']): string {
    return {
      ...
      documento: 'bg-cyan-50 text-cyan-700 ring-cyan-600/20 dark:bg-cyan-500/10 dark:text-cyan-300'
    }[type];
  }
  ...
  private reload(): void {
    ...
    this.alertasService.getMetrics().subscribe((metrics) => this.metrics = metrics);
  }
